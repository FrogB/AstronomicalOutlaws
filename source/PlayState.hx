...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['Outlaw', 0.2], //From 0% to 19%
		['Worst', 0.4], //From 20% to 39%
		...
		['Astronomical', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	var funnyFloatyBoys:Array<String> = ['pcman', 'pcman-angry', 'pc-man-expunged', 'power-pc-man']; //haha funny float script go brrrrrrrrrrrrrrr -frogb
	...
	var blackScreenMadeForCapacitor:FlxSprite;
	...
	override public function create()
	{
		...
		blackScreenMadeForCapacitor = new FlxSprite(-120, -120).makeGraphic(Std.int(FlxG.width * 100), Std.int(FlxG.height * 150), FlxColor.BLACK);
		blackScreenMadeForCapacitor.scrollFactor.set();
		blackScreenMadeForCapacitor.alpha = 0;
		add(blackScreenMadeForCapacitor);
		...
		if(SONG.stage == null || SONG.stage.length < 1) {
			switch (songName)
			{
				case 'wiring' | 'capacitor': //idk tbh
					curStage = '3dWires';
				case 'voltage':
					curStage = '3dMalfunction';
				case 'power!': //duh
					curStage = '3dPower';
		...
		switch (curStage)
		{
			case '3dWires': //not sure if i did this right but if it works it works ig
			//my brother in christ pianoo it already READS THE FUCKING VALUES IN THE BRACKET- -frogb
			{
				defaultCamZoom = 0.72;
				curStage = '3dWires';
				var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('pcman/3dWires'));
			...
			case '3dMalfunction':
			{
				defaultCamZoom = 0.72;
				curStage = '3dMalfunction';
				var bg:FlxSprite = new FlxSprite(-800, -200).loadGraphic(Paths.image('pcman/3dMalfunction')); //IT STILL SHOWS THE BORDERS IDK WHY
				bg.antialiasing = true;
				bg.scrollFactor.set(0.6, 0.6);
				bg.scale.set(2, 2); //i did a golden apple lmao
				bg.active = true;

				add(bg);

				var testshader:Shaders.GlitchEffect = new Shaders.GlitchEffect(3, 10, 0.25); //made it more "malfunctioned".
				bg.shader = testshader.shader;
				curbg = bg;
			}
			...
			case '3dPower':
			{
				defaultCamZoom = 0.72;
				curStage = '3dPower';
				var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('pcman/3dPower'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.3, 0.3);
				bg.scale.set(1.5, 1.5);
				bg.active = true;

				var bg2:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('pcman/powerland'));
				...
				var testshader:Shaders.GlitchEffect = new Shaders.GlitchEffect(3.425, 5, 0.1);
				bg.shader = testshader.shader;
				curbg = bg;
			}
			...
			case '3dBurger':
			{
				...
				var testshader:Shaders.GlitchEffect = new Shaders.GlitchEffect(2, 5, 0.1);
				testshader.waveSpeed = (2.0);
				testshader.waveFrequency = (5.0);
		...
		//DEVASTATION *augh sound effect*

		var songCredit:String;
		switch (SONG.song.toLowerCase())
		{
			case 'wiring' | 'capacitor':
				songCredit = 'Pianoo';
			case 'beefsteak':
				songCredit = 'Ayo';
			case 'malfunction' | 'voltage' | 'devastation' | 'leftover':
				songCredit = 'PG24';
			case 'power!':
				songCredit = 'RyanXD';
			case 'pre-heat' | 'pre heat' | 'preheat' :
				songCredit = 'Deebeezy';
			default:
				songCredit = '';
		}
		
		songTxt = new FlxText(4, textYPos, 0,SONG.song + " - " + songCredit, 16);
		songTxt.setFormat(Paths.font("comic-sans.ttf"), 17, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		songTxt.scrollFactor.set();
		songTxt.borderSize = 1.6;
		songTxt.antialiasing = true;
		songTxt.visible = !ClientPrefs.hideHud;
		add(songTxt);

		//remove this after development is over!
		engineTxt = new FlxText(4, textYPosAlt, 0, "ASTRONOMICAL OUTLAWS | DEVBUILD NO. 6", 14);
		engineTxt.setFormat(Paths.font("comic-sans.ttf"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		engineTxt.scrollFactor.set();
		engineTxt.borderSize = 1.25;
		//engineTxt.antialiasing = true; what- DONT ANTIALIASE IT GOD DAMN IT.
		...
		blackScreenMadeForCapacitor.cameras = [camHUD];
  ...
  public function clearShaderFromCamera(cam:String){
	...
	public function updateScore(miss:Bool = false)
	{
		...
		if(cpuControlled)
		{
			scoreTxt.text = 'DEVIOUS ACT DETECTED!!! | BotPlay';
		}
	...
	switch (SONG.song.toLowerCase())
			{
			case 'capacitor': //you liar... YOU LIAR. AAAAAAAAAAAAAAAAA-
				switch(curBeat)
				{
					case 288:
						FlxTween.tween(blackScreenMadeForCapacitor, {alpha: 1}, 11);
					case 320: // great job bitch you made him mad
						FlxTween.tween(blackScreenMadeForCapacitor, {alpha: 0}, 10);
				}
	}
...
for (key => value in luaShaders)
{
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		totalNotesHit += daRating.ratingMod; //wtf does this do?
	...
	{
		if (SONG.song.toLowerCase() == 'wiring')
		{
			switch (curStep)
			{
				case 128 | 256 | 512 | 639 | 896:
	...
	override function beatHit()
	{
		...
		if (curBeat % 2 == 0) //i did a dave engine uhh thing.
		{
