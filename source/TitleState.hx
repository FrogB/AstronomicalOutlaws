...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		logoBl.animation.addByPrefix('bump', 'bumpthe funny', 24, false);
		logoBl.animation.play('bump');
		logoBl.updateHitbox();

		logoPC = new FlxSprite(-40, 25);
		logoPC.frames = Paths.getSparrowAtlas('logo-pc');

		logoPC.antialiasing = ClientPrefs.globalAntialiasing;
		logoPC.animation.addByPrefix('dance', 'idle', 24, false);
		logoPC.animation.play('dance');
		logoPC.updateHitbox();
		...
		add(logoPC);
		logoPC.shader = swagShader.shader;
	...
	override function beatHit()
	{
		...
		if(logoPC != null)
			logoPC.animation.play('dance', true);
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				// credTextShit.text += '\nNight';
				case 16:
					addMoreText('Astronomical Outlaws'); // credTextShit.text += '\nFunkin';
	...
	function skipIntro():Void
	{
		if (!skippedIntro)
		{
			...
			else //Default! Edit this one!!
			{
				...
				FlxTween.tween(logoBl,{x: 15}, 1.4, {ease: FlxEase.expoInOut}); //KADE ENGINEEEEEEEEEEEEE
				logoBl.angle = -4;
